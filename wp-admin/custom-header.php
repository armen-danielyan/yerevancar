	...
	public function show_header_selector( $type = 'default' ) {
		...
		foreach ( $headers as $header_key => $header ) {
			...
			if ( !empty( $header['attachment_id'] ) )
				$width = ' width="230"';
			echo '<images src="' . set_url_scheme( $header_thumbnail ) . '" alt="' . esc_attr( $header_alt_text ) .'" title="' . esc_attr( $header_desc ) . '"' . $width . ' /></label>';
			echo '</div>';
		}
...
<script type="text/javascript">
	...
	jQuery(document).ready(function() {
		...
		var ximg = jQuery('images#upload').width();
		var yimg = jQuery('images#upload').height();
		...
		jQuery('images#upload').imgAreaSelect({
			handles: true,
			keys: true,
			show: true,
			x1: 0,
