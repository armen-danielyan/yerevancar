...
class WP_Press_This {
	...
	public function side_load_images( $post_id, $content = '' ) {
		...
		if ( preg_match_all( '/<images [^>]+>/', $content, $matches ) && current_user_can( 'upload_files' ) ) {
			foreach ( (array) $matches[0] as $image ) {
				...
				if ( ! is_wp_error( $new_src ) ) {
					// Replace the POSTED content <images> with correct uploaded ones.
	...
	public function source_data_fetch_fallback( $url, $data = array() ) {
		...
		// Fetch and gather <images> data.
		if ( empty( $data['_images'] ) ) {
			$data['_images'] = array();
		}

		if ( preg_match_all( '/<images [^>]+>/', $source_content, $matches ) ) {
			$items = $this->_limit_array( $matches[0] );
		...
		<div class="editor-wrapper">
			...
			<div id="app-container" class="editor">
				...
				wp_editor( $post_content, 'pressthis', array(
					...
					'quicktags' => array(
						'buttons' => 'strong,em,link,block,del,ins,images,ul,ol,li,code,more',
					),
				) );
