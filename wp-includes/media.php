...
/**
 * Gets an images tag for an image attachment, scaling it down if requested.
 *
...
function get_image_tag( $id, $alt, $title, $align, $size = 'medium' ) {
	...
	$html = '<images src="' . esc_attr($img_src) . '" alt="' . esc_attr($alt) . '" ' . $title . $hwstring . 'class="' . $class . '" />';
...
/**
 * Get an HTML images element representing an image attachment
 *
 ...
 * @return string HTML images element or empty string on failure.
 */
...
/**
 * Filters 'images' elements in post content to add 'srcset' and 'sizes' attributes.
 *
 * @since 4.4.0
 *
...
function wp_make_content_images_responsive( $content ) {
	if ( ! preg_match_all( '/<images [^>]+>/', $content, $matches ) ) {
		return $content;
	}
...
/**
 * Adds 'srcset' and 'sizes' attributes to an existing 'images' element.
 *
 * @since 4.4.0
 *
 * @see wp_calculate_image_srcset()
 * @see wp_calculate_image_sizes()
 *
 * @param string $image         An HTML 'images' element to be filtered.
 * @param array  $image_meta    The image meta data as returned by 'wp_get_attachment_metadata()'.
 * @param int    $attachment_id Image attachment ID.
 * @return string Converted 'images' element with 'srcset' and 'sizes' attributes added.
 */
function wp_image_add_srcset_and_sizes( $image, $image_meta, $attachment_id ) {
	...
	if ( $srcset && $sizes ) {
		...
		$image = preg_replace( '/<images ([^>]+?)[\/ ]*>/', '<images $1' . $attr . ' />', $image );
	}
...
function img_caption_shortcode( $attr, $content = null ) {
	...
	if ( ! isset( $attr['caption'] ) ) {
		if ( preg_match( '#((?:<a [^>]+>\s*)?<images [^>]+>(?:\s*</a>)?)(.*)#is', $content, $matches ) ) {
			$content = $matches[1];
